...
export const configureRoutes = (app: Express) => {
    const validMethods: ExpressMethod[] = ['get', 'post', 'put', 'delete', 'patch', 'options', 'head'];
    ...
    Object.entries(config.services).forEach(([name, service]) => {
        ...
        service.routes.forEach((route) => {
            route.methods.forEach((method: string) => {
                ...
                if (validMethods.includes(expressMethod)) {
                    const handler = createHandler(hostname, route.path, method);
                    app[expressMethod](`/api/${route.path}`, handler);
                } else {
                    console.error(`Invalid HTTP method: "${method}" for route ${route.path} in service ${name}`);
                }
            });
        });
    });
