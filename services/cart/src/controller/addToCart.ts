...
//         // check if cart session id is present in the request header exists in the store

//         if (cartSessionId) {
//             const existingCart = await redis.get(`cart:${cartSessionId}`);
...
//         if (!cartSessionId) {
//             cartSessionId = uuid();
//             console.log("create new session id", cartSessionId);

//             //set the cart session id in the redis store
//             await redis.setex(`cart:${cartSessionId}`, CART_TTL, cartSessionId);
...
//         //TODO: chekc inventory for availability
